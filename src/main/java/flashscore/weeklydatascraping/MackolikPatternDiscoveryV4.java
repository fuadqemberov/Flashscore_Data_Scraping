...
public class MackolikPatternDiscoveryV4 {
    ...
    static class Match {
        ...
        static String getNormalizedScore(String score) {
            ...
            try {
                ...
                // Orijinal sıralaması korunur (home-away olarak); sadece görece fark gösterilir
                // Örn: 1-0 → "1-0", 0-1 → "0-1", 1-3 → "1-3", 0-0 → "0-0"
                // Bu sayede rotasyon deseni korunur (aynı desen hangi sırada görülürse öyle kalır)
                return a + "-" + b;
            } catch (Exception e) {
                return null;
            }
    ...
    static class DiscoveredPattern {
        ...
        // YENİ: Dörtlü çevrim için gözlemlenen 4'lü skor dizileri (örnek: [["1-1","1-3","0-1","0-0"], ...])
        List<List<String>> observedSequences = new ArrayList<>();
        // YENİ: dörtlü grubunun order-insensitive temsilcisi (sadece skor multiset'i)
        String bagKey = "";
    ...
    public MackolikPatternDiscoveryV4() {
    ...
    // YENİ: Dörtlü (4 maç) döngü/cycle pattern'leri keşfet
    // MANTIK: Her takım için ardışık 4 maçlık skorları al.
    // KURAL: Aynı 4 skor (SİRASI FARKLI OLSA BİLE) en az 2 defa gözlenmişse PATTERN'dir.
    // Örn: [1-0, 0-1, 0-1, 0-0] = [0-1, 0-1, 0-0, 1-0] (aynı pattern, rotasyonu farklı)
    public List<DiscoveredPattern> discoverQuartetCyclePatterns(List<Match> matches) {
        System.out.println("🔍 DÖRTLÜ ÇEVRİM (4-lü) PATTERN'LERİ ARANIYOR - MULTISET BAZLI (SIRASI FARKLIYSA DA AYNI) (ÇOK SEZONLU)...\n");

        // key = bagKey (sorted multiset: sıra fark etmez, 4 skor aynıysa aynı key)
        // value = list of observed sequences (farklı rotasyonlarda gözlenenler)
        Map<String, List<List<String>>> keyToSeqs = new HashMap<>();
        Map<String, Set<String>> keyToSeasons = new HashMap<>();
        Map<String, List<String>> keyToExamples = new HashMap<>();
        ...
        for (String team : teamMatches.keySet()) {
            ...
            for (int i = 0; i + 4 <= normScores.size(); i++) {
                List<String> window = normScores.subList(i, i + 4);
                if (window.stream().anyMatch(Objects::isNull)) continue;

                // Bag key: multiset (sıra fark etmez) - sorted order'da aynı 4 skor = aynı bag
                List<String> sorted = new ArrayList<>(window);
                Collections.sort(sorted);
                String bagKey = String.join("|", sorted);

                // DEBUG: log the window being added
                System.out.println("[DEBUG] add window for team='" + team + "' season='" + seasonsList.get(i) + "' window='" + String.join(", ", window) + "' bagKey='" + bagKey + "'");

                keyToSeqs.computeIfAbsent(bagKey, k -> new ArrayList<>()).add(new ArrayList<>(window));
                keyToSeasons.computeIfAbsent(bagKey, k -> new HashSet<>()).add(seasonsList.get(i));

                String example = team + " | Sezon: " + seasonsList.get(i) + " → " + String.join(", ", window);
                keyToExamples.computeIfAbsent(bagKey, k -> new ArrayList<>()).add(example);
            }
        }

        List<DiscoveredPattern> discovered = new ArrayList<>();
        for (String key : keyToSeqs.keySet()) {
            List<List<String>> seqs = keyToSeqs.get(key);
            Set<String> seasons = keyToSeasons.getOrDefault(key, Collections.emptySet());

            // KURAL: Aynı 4-skor bag'i (multiset) en az 2 defa görüldüyse pattern'dir
            // SİRASI FARKLI OLSA BİLE, 4 skor aynıysa aynı pattern
            if (seqs.size() >= 2) {
                DiscoveredPattern pattern = new DiscoveredPattern("DORTLU_CYCLE", key, 0, "");
                pattern.bagKey = key; // store bagKey (multiset)
                pattern.foundCount = seqs.size();
                pattern.totalChecked = seqs.size();
                pattern.calculate();
                pattern.seasons = new ArrayList<>(seasons);
                pattern.examples = keyToExamples.getOrDefault(key, new ArrayList<>());

                // Store unique observed sequences
                Set<String> uniq = new HashSet<>();
                for (List<String> s : seqs) {
                    String rep = String.join("|", s);
                    if (!uniq.contains(rep)) {
                        uniq.add(rep);
                        pattern.observedSequences.add(new ArrayList<>(s));
                    }
                }

                // DEBUG: print candidate bagKey and some info
                System.out.println("[DEBUG] DORTLU_CYCLE PATTERN BULUNDU! bagKey='" + key + "' occurrences=" + seqs.size() + " (farklı rotasyonlar sayılı) seasons=" + seasons.size());
                System.out.println("[DEBUG]   examples: " + String.join(" || ", keyToExamples.getOrDefault(key, Collections.emptyList())));
    ...
    private boolean analyzePatternsForMatch(String homeTeam, String awayTeam, int week, String season,
        ...
        for (String team : Arrays.asList(homeTeam, awayTeam)) {
            ...
            // 3. DÖRTLÜ CYCLE (4-lü çevrim) pattern kontrolü: son 3 maça bak
            // Multiset mantığı: Eğer son 3 skor, pattern'in 4'lüsünün 3 öğesini içeriyorsa
            // eksik 4. element'i tahmin et (sıra önemli değil)
            if (teamMatches.size() >= 3) {
                ...
                if (last3Norm.size() == 3) {
                    for (DiscoveredPattern p : patterns) {
                        ...
                        // p.bagKey = sorted multiset (ör. "0-0|0-1|1-0|3-1")
                        // p.observedSequences = gözlenen diziler (farklı rotasyonlar)
                        ...
                        // Compute multiset difference: bag - last3 = missing element(s)
    ...
    public static void main(String[] args) {
        MackolikPatternDiscoveryV4 analyzer = new MackolikPatternDiscoveryV4();
